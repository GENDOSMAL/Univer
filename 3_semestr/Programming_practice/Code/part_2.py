...
def PrepareStackToCalc(parsedFormula):  # Некоторый алгоритм сортировочной станции
    ...
    for token in parsedFormula:
        ...
        if token in operators:
            ...
            # Если элемент - закрывающая скобка, выдаём все элементы из стека, до открывающей скобки,а открывающую скобку выкидываем из стека.
            while localStack:
                x = localStack.pop()
                if x == "(":
                    break
