...
app.get('/main', function (req, res) {
  ...
  resut += "Создаем дерево размера [" + len + "]<br>"
  for (let i = 0; i < len; i++) {
    tree.insert(getRandomInt(99))
  }
  tree.insert(50)
  resut += "Все остальное в консоли<br>"
  logtree();

  var element=getRandomInt(999)
  resut += "Вставим некторый элемент "+element+"<br>"
  tree.insert(element)
  console.log("Вывод после вставки:")
  logtree();

  var minElement=tree.getMin()
  resut += "Удалим минимальный элемент "+minElement.element+"<br>"
  tree.delete(minElement)
  console.log("Вывод после удаления минимального элемента:")
  logtree();

  var searchedItems=tree.search(50)
  resut+="Нашли ранее добавленный элемент ["+searchedItems.element+"]<br>";
  console.log("Нашли ранее добавленный элемент ["+searchedItems.element+"]")
  str = JSON.stringify(searchedItems, null, 4);
  console.log(str)
  res.send(resut + "");
